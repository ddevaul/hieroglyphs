...
const setShuffle = (glyphs, shuffle, setShuffledGlyphs, setCompleted) => {
    let shuffledGlyphs = shuffle(glyphs);
    setShuffledGlyphs(shuffledGlyphs);
    setCompleted(false);
}
...
export default Practice = ({navigation}) => {
  ...
  if (completed) {
    return(
      <View>
        <Text>Look at You Smarty! You Completed Everything!!</Text>
        <Button title="Go Again?" onPress={() => setShuffle(glyphs, shuffle, setShuffledGlyphs, setCompleted)}></Button>
      </View>
    )
  }
    ...
    return (
      <View>
          {/* <Button title="Shuffle" onPress={() => s}></Button> */}

          <SketchPad glyphs={shuffledGlyphs} setCompleted={setCompleted}></SketchPad>
      </View>
